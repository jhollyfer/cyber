...
## Modelo Storage (Prisma)

O sistema mantem um modelo Prisma para rastrear metadados de cada arquivo armazenado na tabela `storages`:

| Campo          | Tipo     | Descricao                                    |
|----------------|----------|----------------------------------------------|
| `id`           | `String` | UUID gerado automaticamente                  |
| `url`          | `String` | URL de acesso ao arquivo (ex: `/storage/...`)|
| `filename`     | `String` | Nome do arquivo no disco (com ID aleatorio)  |
| `mimetype`     | `String` | Tipo MIME do arquivo (ex: `image/webp`)      |
| `original_name`| `String` | Nome original do arquivo enviado pelo usuario|
| `size`         | `Int`    | Tamanho do arquivo em bytes                  |

O modelo Storage possui relacoes com diversas entidades: avatar de artesao, prova de afiliacao, avatar de curador, prova de credencial, imagens de pecas, imagens de solicitacoes de pecas, imagens de categorias e solicitacoes de atualizacao.
...
6. Os metadados sao persistidos no modelo `Storage` via Prisma
