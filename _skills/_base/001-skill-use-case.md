...
2. **Constructor com repository injetado (contract, nao implementacao)** -- o constructor recebe o contract abstrato (ex.: `UserContractRepository`), nunca a implementacao concreta (ex.: `UserPrismaRepository`). A resolucao e feita pelo DI Registry.
...
| Repository concreto no constructor | Injeta `UserPrismaRepository` em vez de `UserContractRepository` | O use case so deve conhecer o contract; a resolucao e responsabilidade do DI Registry (ver `011-skill-di-registry.md`) |
