...
export const UserShowParamSchema = z.object({
  id: z.string({ message: 'O ID e obrigatorio' }).min(1, 'O ID e obrigatorio').trim(),
});
...
1. `UserShowParamSchema` valida o parametro `id` da URL -- e um schema simples com `.string()`, `.min(1)` e `.trim()`.
