...
import [Entity]MemoryRepository from '@application/repositories/[entity]/[entity]-memory.repository';
import [Entity][Action]UseCase from './[action].use-case';

let [entity]MemoryRepository: [Entity]MemoryRepository;
let sut: [Entity][Action]UseCase;

describe('[Entity] [Action] Use Case', () => {
  beforeEach(() => {
    [entity]MemoryRepository = new [Entity]MemoryRepository();
    sut = new [Entity][Action]UseCase([entity]MemoryRepository);
  });

  it('deve [descricao do cenario de sucesso]', async () => {
    // Arrange: preparar dados necessarios
    const created = await [entity]MemoryRepository.create({
      // ...campos da entidade
    });
  ...
  it('deve retornar erro [CAUSA] (500) em falha de DB', async () => {
    ...
    vi.spyOn([entity]MemoryRepository, '[metodo]').mockRejectedValueOnce(new Error('Database error'));
...
```typescript
import { beforeEach, describe, expect, it, vi } from 'vitest';
import UserMemoryRepository from '@application/repositories/user/user-memory.repository';
...
describe('User Show Use Case', () => {
  beforeEach(() => {
    ...
    sut = new UserShowUseCase(userMemoryRepository);
  });

  it('deve retornar usuario quando encontrado', async () => {
    const created = await userMemoryRepository.create({
  ...
  it('deve retornar erro GET_USER_BY_ID_ERROR (500) em falha de DB', async () => {
    vi.spyOn(userMemoryRepository, 'findBy').mockRejectedValueOnce(new Error('Database error'));
...
1. **NUNCA usar banco de dados real** - Sempre utilizar repositorios memory (`[Entity]MemoryRepository`). Testes unitarios devem ser rapidos, isolados e sem dependencias externas.
