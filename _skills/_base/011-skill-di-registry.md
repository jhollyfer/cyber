...
import {{Entity}}PrismaRepository from '@application/repositories/{{entity}}/{{entity}}-prisma.repository';
...
export function registerDependencies(): void {
  // Repositories
  injectablesHolder.injectService({{Entity}}ContractRepository, {{Entity}}PrismaRepository);
...
import UserPrismaRepository from '@application/repositories/user/user-prisma.repository';
...
2. `UserPrismaRepository` e a implementacao concreta que usa Prisma/PostgreSQL.
3. `injectablesHolder.injectService(Contract, Implementacao)` registra o binding: sempre que o container resolver `UserContractRepository`, ele entregara uma instancia de `UserPrismaRepository`.
