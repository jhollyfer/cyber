...
|---|---|---|
| `dev` | `vite dev --port 3000` | Servidor de desenvolvimento com HMR (porta 3000) |
| `build` | `NODE_OPTIONS='--max-old-space-size=4096' vite build` | Build de producao com memoria extra |
...
**Nota:** Nao ha configuracao `server.port` no vite.config.ts. A porta 3000 e definida no script `dev` do package.json.
...
**Nota:** O export e `env` (minusculo), nao `Env`.
...
| `VITE_API_BASE_URL` | URL | `http://localhost:4000` | URL base da API backend |
