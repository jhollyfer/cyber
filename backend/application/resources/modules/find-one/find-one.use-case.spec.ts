...
let useCase: FindOneModuleUseCase;
let moduleRepository: ModuleMemoryRepository;

describe('Find One Module Use Case', () => {
  beforeEach(() => {
    moduleRepository = new ModuleMemoryRepository();
    useCase = new FindOneModuleUseCase(moduleRepository as any);
  });

  describe('execute', () => {
    it('deve retornar um modulo pelo id com sucesso', async () => {
      ...
      if (result.isRight()) {
        expect(result.value.id).toBe(module.id);
        expect(result.value.title).toBe('Modulo 1');
      }
    });
    ...
    it('deve retornar erro 500 quando ocorrer erro no repositorio', async () => {
      ...
      if (result.isLeft()) {
        ...
        expect(result.value.cause).toBe('FIND_ONE_MODULE_ERROR');
      }
    });
  });
});
