...
describe('Finish Session Use Case', () => {
    beforeEach(() => {
        ...
        useCase = new FinishSessionUseCase(gameSessionRepository, questionRepository, moduleRepository);
    });
    describe('execute', () => {
        ...
        it('deve retornar erro 500 quando ocorrer erro no repositorio', async () => {
            vi.spyOn(gameSessionRepository, 'findById').mockRejectedValue(new Error('Database error'));
            const result = await useCase.execute({
                session_id: 'any-id',
                user_id: 'user-123',
            });
