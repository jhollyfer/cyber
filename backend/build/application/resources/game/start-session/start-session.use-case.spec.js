...
describe('Start Session Use Case', () => {
    beforeEach(() => {
        ...
        useCase = new StartSessionUseCase(gameSessionRepository, moduleRepository, questionRepository);
    });
    describe('execute', () => {
        it('deve iniciar uma sessao de jogo com sucesso', async () => {
            ...
            if (result.isRight()) {
                ...
                expect(result.value.questions[0].correct).toBeUndefined();
                expect(result.value.questions[0].explanation).toBeUndefined();
            }
        });
