...
// application/resources/stats/export-csv/export-csv.controller.ts
...
var _class = class {
  ...
  useCase;
  constructor(useCase = getInstanceByToken(ExportCsvUseCase)) {
  ...
  async handle(_request, response) {
    ...
    response.header("Content-Type", "text/csv; charset=utf-8").header("Content-Disposition", 'attachment; filename="cyberguardian-ranking.csv"').status(200).send(result.value);
  }
};
_ts_decorate([
  GET({
...
_class = _ts_decorate([
  ...
  _ts_metadata("design:type", Function),
  _ts_metadata("design:paramtypes", [
    typeof ExportCsvUseCase === "undefined" ? Object : ExportCsvUseCase
  ])
], _class);
export {
  _class as default
