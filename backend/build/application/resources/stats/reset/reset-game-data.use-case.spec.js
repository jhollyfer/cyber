...
describe('Reset Game Data Use Case', () => {
    beforeEach(() => {
        ...
        useCase = new ResetGameDataUseCase(statsRepository);
    });
    describe('execute', () => {
        it('deve resetar os dados de jogo com sucesso', async () => {
            ...
            if (result.isRight()) {
                expect(result.value.message).toBe('Todos os dados de jogo foram resetados com sucesso.');
            }
        });
        it('deve retornar erro 500 quando ocorrer erro no repositorio', async () => {
            vi.spyOn(statsRepository, 'resetGameData').mockRejectedValue(new Error('Database error'));
            const result = await useCase.execute();
